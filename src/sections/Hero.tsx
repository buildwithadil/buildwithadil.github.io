import type { CSSProperties } from 'react'
import { Link } from 'react-router-dom'
import { Badge, Button, Container } from '../ui'
import Backdrop from '../components/Backdrop'
import Magnetic from '../components/Magnetic'
import Headshot from '../components/Headshot'

const delay = (ms: number) => ({ '--enter-delay': `${ms}ms` }) as CSSProperties

/**
 * Split hero — a large type moment beside a framed portrait. Headline animates
 * transform-only (LCP-safe); the rest fades + rises in a subtle stagger.
 */
export default function Hero() {
  return (
    <section className="relative overflow-hidden border-b border-border">
      <Backdrop />
      <Container className="py-20 lg:py-28">
        <div className="grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16">
          {/* Copy */}
          <div className="max-w-2xl">
            <div className="animate-in" style={delay(0)}>
              <Badge>
                <span
                  className="size-1.5 rounded-full bg-emerald-500"
                  aria-hidden
                />
                Available for new projects
              </Badge>
            </div>

            <h1
              className="animate-rise mt-6 text-balance text-5xl font-semibold leading-[1.02] tracking-[-0.03em] sm:text-6xl lg:text-[4.25rem]"
              style={delay(80)}
            >
              Websites and platforms,{' '}
              <span className="text-gradient">engineered to perform.</span>
            </h1>

            <p
              className="animate-in mt-7 max-w-xl text-lg leading-relaxed text-fg-muted"
              style={delay(220)}
            >
              I’m Adil Shaikh — I help academies and ambitious teams turn ideas
              into fast, SEO-strong products people trust.
            </p>

            <div
              className="animate-in mt-9 flex flex-wrap items-center gap-4"
              style={delay(320)}
            >
              <Magnetic>
                <Button asChild size="lg">
                  <Link to="/contact">Start a project</Link>
                </Button>
              </Magnetic>
              <Button asChild variant="secondary" size="lg">
                <Link to="/work">See selected work</Link>
              </Button>
            </div>

            <p
              className="animate-in mt-6 text-sm text-fg-subtle"
              style={delay(400)}
            >
              No obligation. I usually reply within a day.
            </p>
          </div>

          {/* Portrait */}
          <div
            className="animate-in relative mx-auto w-full max-w-sm lg:mx-0"
            style={delay(240)}
          >
            <div
              aria-hidden
              className="absolute -inset-3 -z-10 rounded-[1.75rem] border border-border"
            />
            <div className="overflow-hidden rounded-3xl border border-border bg-surface shadow-xl">
              <Headshot />
            </div>
          </div>
        </div>
      </Container>
    </section>
  )
}
